...
class FCMService {
  ...
  static Future<void> initializeFirebase() async {
    try {
      if (Firebase.apps.isEmpty) {
        ...
        await Firebase.initializeApp(
          name: AppConstants
              .appName, //Platform.isAndroid ? AppConstants.appName : '[DEFAULT]',
    ...
    // Запрашиваю разрешения на показ уведомлений, если их еще нет
    NotificationSettings settings =
        await _firebaseMessaging!.getNotificationSettings();
    if (settings.authorizationStatus == AuthorizationStatus.notDetermined) {
      // authorized : Пользователь предоставил разрешение.
      // denied : Пользователь отклонил разрешение.
      // notDetermined : пользователь еще не решил, следует ли предоставлять разрешение.
  ...
  static Future<void> onBackgroundMsg(
      Function(RemoteMessage message)? onBackgroundMessage) async {
  ...
  static Future<void> onMessage(
      Function(RemoteMessage message)? onShowMessage) async {
