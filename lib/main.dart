...
void onPayload(String? payload) {
  ...
  switch (data['EntityType']) {
    case EntityType.newMedcardEvent:
      if (userId != null) {
        _router.push(MedcardRoute(
            userId: userId, isChildrenPage: true, eventId: data['EventId']));
      }
      break;
      ...
      if ((parts?.length ?? 0) > 1) {
        try {
          ...
          Sentry.captureMessage(
              "Push message: $message $dateString ${dateFormat.parse(dateString)}");
          final date = dateFormat.parse(dateString);
          pushNavigationService.nextPage = AppointmentsRoute(initDay: date);
